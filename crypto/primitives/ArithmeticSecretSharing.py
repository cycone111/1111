...
from configs.base_config import RING_MAX
...
def beaver_mul(beaver_provider, x: ArithmeticSecretSharing, y: ArithmeticSecretSharing):
    ...
    if x.party.party_id == 0:
        sign = 1
    sign = RingTensor.convert_to_ring(torch.tensor(sign))
    res = c.ring_tensor + plaintext_e * b.ring_tensor + a.ring_tensor * plaintext_f + plaintext_e * plaintext_f * sign
